# Zenbot Strategie: Golden-EA-Zen (Adaptierte Version)

## 1. Strategiebeschreibung

Die Strategie **Golden-EA-Zen** ist eine für Zenbot adaptierte Version des ursprünglichen MT4 Expert Advisors "Golden EA". Der ursprüngliche EA war auf den Handel mit Gold-Paaren (XAUUSD, XAUEUR) spezialisiert und nutzte eine **Korrelations- und Momentum-Logik** in Kombination mit einer **Martingale-Lot-Erhöhung**.

Da Zenbot primär für den Handel mit Kryptowährungen auf einem einzelnen Marktpaar konzipiert ist und keine nativen Multi-Währungs-Funktionen bietet, wurde die Kernidee in eine Zenbot-kompatible Momentum-Strategie überführt.

Die Strategie zielt darauf ab, **starke Momentum-Bewegungen** zu identifizieren und in deren Richtung zu handeln, ähnlich wie der Best-Grider, jedoch mit anderen Standard-Parametern.

### Kernlogik

1.  **Momentum-Filter (Ersatz für Korrelation):** Die Strategie prüft, ob die letzte abgeschlossene Kerze eine **Mindestgröße** (`min_pips_filter`) überschritten hat und eine **klare Richtung** (bullish oder bearish) aufweist. Dies simuliert die ursprüngliche Logik, die nur bei starker, koordinierter Marktbewegung handelte.
...
| `min_pips_filter` | Mindest-Pips-Bewegung des letzten Bars als Filter (entspricht MinPips) | Number | `250` |
...
```bash
./zenbot.sh backfill --days 7 BTC-USD
./zenbot.sh backtest BTC-USD --strategy=golden-ea-zen --period=1m --min_pips_filter=250 --martingale_factor=1.2
```

## 3. Implementierungsdetails (strategy.js)

Die Logik ist in der Datei `Golden-EA-Zen.js` implementiert.

```javascript
// Auszug aus Golden-EA-Zen.js
// ...
