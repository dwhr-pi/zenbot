# Zenbot Strategie: Best-Grider-Zen (Adaptierte Version)

## 1. Strategiebeschreibung

Die Strategie **Best-Grider-Zen** ist eine für Zenbot adaptierte Version des ursprünglichen MT4 Expert Advisors "Best-Grider". Der ursprüngliche EA basierte auf einer **Multi-Währungs-Korrelationsanalyse** und einer **Martingale-Grid-Logik**. Da Zenbot primär für den Handel mit Kryptowährungen auf einem einzelnen Marktpaar konzipiert ist und keine nativen Grid- oder Multi-Währungs-Funktionen bietet, wurde die Kernidee in eine Zenbot-kompatible Momentum-Strategie überführt.
...
| :--- | :--- | :--- | :--- |
| `period` | Zeitrahmen der Kerzen (z.B. 1m, 5m, 15m) | String | `5m` |
| `momentum_period` | Periodenlänge für den internen EMA-Check (wird für die Berechnung verwendet, ist aber nicht der Hauptfilter) | Number | `10` |
| `momentum_threshold` | Mindestprozentsatz der Kursbewegung für ein Signal (z.B. 0.1 für 0.1%) | Number | `0.1` |
| `martingale_factor` | Faktor zur Erhöhung der Lot-Größe nach einem Verlust (1.0 = keine Erhöhung) | Number | `1.2` |
| `min_pips_filter` | Mindest-Pips-Bewegung des letzten Bars als Filter (entspricht MinPips) | Number | `25` |

### Beispiel für die Zenbot-Befehlszeile (Backtesting)

```bash
./zenbot.sh backfill --days 90 ETH-USD
./zenbot.sh backtest ETH-USD --strategy=best-grider-zen --period=5m --momentum_period=10 --martingale_factor=1.2 --min_pips_filter=25
```

## 3. Implementierungsdetails (strategy.js)

Die Logik ist in der Datei `Best-Grider-Zen.js` implementiert.

```javascript
// Auszug aus Best-Grider-Zen.js
// ...
    onPeriod: function (s, cb) {
      ...
      s.options.size = s.options.size * lot_size_factor // ACHTUNG: Dies ist eine vereinfachte Anpassung
